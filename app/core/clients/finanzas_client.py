"""Cliente al core Nivel 1 'finanzas' (ledger interno por cliente)."""
...
class FinanzasClient:
    ...
    async def get_balance(self, account_id: str) -> dict[str, Any]:
        return await self.c.get(f"/v1/accounts/{account_id}/balance")

    async def post_charge(
        self, account_id: str, *, amount_cents: int, concept: str, reference: str
    ) -> dict[str, Any]:
        return await self.c.post(
            f"/v1/accounts/{account_id}/charges",
            json={"amount_cents": amount_cents, "concept": concept, "reference": reference},
        )
    ...
    async def post_credit(
        ...
        return await self.c.post(
            f"/v1/accounts/{account_id}/credits",
            json={"amount_cents": amount_cents, "concept": concept, "reference": reference},
        )
